...
class DribbleModel(Model):
    ...
    def build_observation(self, body_pos_2D, body_heading, body_vel_2D, body_omega, 
                          ball_pos_proj, ball_vel_proj, cmd_vel: torch.Tensor, 
                          target_pos, **kwargs) -> torch.Tensor: # type: ignore[override]
        return torch.cat((body_pos_2D, body_heading, body_vel_2D, body_omega,
                          ball_pos_proj, ball_vel_proj, cmd_vel, target_pos), dim=1)

    def build_terminated(self, body_pos, body_pos_2D, 
                         ball_pos_2D, ball_dis, **kwargs
                         ) -> torch.Tensor: # type: ignore[override]
        robot_fall        = body_pos[:, 2] < 0.3
        ball_out_of_range = (torch.abs(ball_pos_2D) > self.terminate_range).any(dim=1)
        body_out_of_range = (torch.abs(body_pos_2D) > self.terminate_range).any(dim=1)
        ball_lost         = ball_dis > self.cfg.ball_loss_distance
        return (robot_fall | ball_out_of_range | body_out_of_range | ball_lost).unsqueeze(1)
    
    def build_truncated(self, **kwargs) -> torch.Tensor:
        return (self.time_steps >= self.cfg.truncated_step_limit).unsqueeze(1)
    
    def build_reward(self, **kwargs) -> torch.Tensor: # type: ignore[override]
        return torch.zeros((self.scene.n_envs, 1), dtype=torch.float, device=gs.device)
    
    def build_info(self, ball_pos, ball_vel, body_pos, cmd_vel, ball_pos_rel,
    ...
    def build_cache(self, body_pos, body_quat, body_lin_vel, body_ang_vel, 
        ...
        self.cache["body_pos_2D"]   = state["body_pos"][:, 0:2].contiguous()
        self.cache["body_vel_2D"]   = state["body_lin_vel"][:, 0:2].contiguous()
        self.cache["body_omega"]    = state["body_ang_vel"][:, 2].unsqueeze(1).contiguous()
        self.cache["ball_pos_rel"]  = state["ball_pos_2D"] - state["body_pos_2D"]
        self.cache["ball_dis"]      = torch.norm(state["ball_pos_rel"], dim=1, keepdim=True)     
        self.cache["body_heading"]  = get_continuous_heading(state["body_quat"])
        self.cache["ball_pos_proj"] = get_ball_relative_pos(**state)
        self.cache["ball_vel_proj"] = get_ball_relative_vel(**state)
