...
class DribbleEnv(Env):
    ...
    def reset(self, envs_idx: torch.Tensor | None = None
        ...
        def rand(pos: torch.Tensor | float, noise: float, dim):
            return pos + noise * torch.randn((envs_idx.shape[0], dim), \
        ...
        robot_pos = ball_pos + robot_pos_r * torch.cat([torch.cos(robot_pos_ang), 
                                                        torch.sin(robot_pos_ang)], dim=1)
        robot_yaw = rand(0.0, torch.pi, 1).squeeze(1)
        robot_quat = torch.zeros((envs_idx.shape[0], 4), device=gs.device, dtype=torch.float)
